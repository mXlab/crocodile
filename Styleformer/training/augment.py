...
@dataclass
class AugmentPipeConfig:
    xflip: float = 0
    rotate90: float = 0
    xint: float = 0
    xint_max: float = 0.125
    scale: float = 0
    rotate: float = 0
    aniso: float = 0
    xfrac: float = 0
    ...
    imgfilter: float = 0
    imgfilter_bands: List[float] = field(default_factory=[1.0, 1, 1, 1].copy)
    imgfilter_std: float = 1
    noise: float = 0
    cutout: float = 0
    noise_std: float = 0.1
    cutout_size: float = 0.5
    aug_linformer: float = 0


class AugmentPipe(torch.nn.Module):
    p: torch.Tensor
    Hz_geom: Any
    Hz_bank: torch.Tensor

    def __init__(self, config: AugmentPipeConfig = AugmentPipeConfig()):
        super().__init__()
        self.register_buffer(
            "p", torch.ones([])
        )  # Overall multiplier for augmentation probability.
        self.aug_linformer = config.aug_linformer

        # Pixel blitting.
        self.xflip = float(config.xflip)  # Probability multiplier for x-flip.
        self.rotate90 = float(
            config.rotate90
        ...
        self.contrast = float(config.contrast)  # Probability multiplier for contrast.
        self.lumaflip = float(config.lumaflip)  # Probability multiplier for luma flip.
        self.hue = float(config.hue)  # Probability multiplier for hue rotation.
        self.saturation = float(
            config.saturation
        )  # Probability multiplier for saturation.
        self.brightness_std = float(
            config.brightness_std
        )  # Standard deviation of brightness.
        self.contrast_std = float(
            config.contrast_std
        )  # Log2 standard deviation of contrast.
        self.hue_max = float(config.hue_max)  # Range of hue rotation, 1 = full circle.
        self.saturation_std = float(
            config.saturation_std
        ...
        self.cutout = float(config.cutout)  # Probability multiplier for cutout.
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.xflip > 0:
            ...
            i = torch.where(
                torch.rand([batch_size], device=device) < self.xflip * self._p,
                i,
                torch.zeros_like(i),
            )
            if debug_percentile is not None:
        ...
        if self.rotate90 > 0:
            ...
            i = torch.where(
                torch.rand([batch_size], device=device) < self.rotate90 * self._p,
                i,
                torch.zeros_like(i),
            )
            if debug_percentile is not None:
        ...
        if self.xint > 0:
            ...
            t = torch.where(
                torch.rand([batch_size, 1], device=device) < self.xint * self._p,
                t,
                torch.zeros_like(t),
            )
            if debug_percentile is not None:
        ...
        if self.scale > 0:
            ...
            s = torch.where(
                torch.rand([batch_size], device=device) < self.scale * self._p,
                s,
                torch.ones_like(s),
            )
            if debug_percentile is not None:
        ...
        p_rot = 1 - torch.sqrt(
            (1 - self.rotate * self._p).clamp(0, 1)
        )  # P(pre OR post) = p
        if self.rotate > 0:
            theta = (
        ...
        if self.aniso > 0:
            ...
            s = torch.where(
                torch.rand([batch_size], device=device) < self.aniso * self._p,
                s,
                torch.ones_like(s),
            )
            if debug_percentile is not None:
        ...
        if self.xfrac > 0:
            ...
            t = torch.where(
                torch.rand([batch_size, 1], device=device) < self.xfrac * self._p,
                t,
                torch.zeros_like(t),
            )
            if debug_percentile is not None:
        ...
        if self.brightness > 0:
            ...
            b = torch.where(
                torch.rand([batch_size], device=device) < self.brightness * self._p,
                b,
                torch.zeros_like(b),
            )
            if debug_percentile is not None:
        ...
        if self.contrast > 0:
            ...
            c = torch.where(
                torch.rand([batch_size], device=device) < self.contrast * self._p,
                c,
                torch.ones_like(c),
            )
            if debug_percentile is not None:
        ...
        if self.lumaflip > 0:
            ...
            i = torch.where(
                torch.rand([batch_size, 1, 1], device=device) < self.lumaflip * self._p,
                i,
                torch.zeros_like(i),
            )
            if debug_percentile is not None:
        ...
        if self.hue > 0 and num_channels > 1:
            ...
            theta = torch.where(
                torch.rand([batch_size], device=device) < self.hue * self._p,
                theta,
                torch.zeros_like(theta),
            )
            if debug_percentile is not None:
        ...
        if self.imgfilter > 0:
            ...
            for i, band_strength in enumerate(self.imgfilter_bands):
                ...
                t_i = torch.where(
                    ...
                    < self.imgfilter * self._p * band_strength,
                    t_i,
                    torch.ones_like(t_i),
                )
                if debug_percentile is not None:
        ...
        if self.noise > 0:
            ...
            sigma = torch.where(
                torch.rand([batch_size, 1, 1, 1], device=device) < self.noise * self._p,
                sigma,
